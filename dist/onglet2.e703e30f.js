    ...
    d3.csv("./TRIP_PART_3.csv").then(function (data3) {
      d3.csv("./Vessel Type Class.csv").then(function (dataTypes) {
        ...
        var vesselLengthChart = new dc.BarChart('#length-chart').width(chartWidth).height(chartHeight).margins({
          ...
          return chartNbBars;
        }).brushOn(false).xAxisLabel('Longueur (m)').elasticY(true).dimension(vesselLength).group(vesselLengths);
        ...
        var vesselWidthChart = new dc.BarChart('#width-chart').width(chartWidth).height(chartHeight).margins({
          ...
          return chartNbBars;
        }).brushOn(false).xAxisLabel('Largeur (m)').elasticY(true).dimension(vesselWidth).group(vesselWidths);
        ...
        var vesselCapacityChart = new dc.BarChart('#capacity-chart').width(chartWidth).height(chartHeight).margins({
          ...
          return chartNbBars;
        }).brushOn(false).xAxisLabel('Capacité (t)').elasticY(true).dimension(vesselCapacity).group(vesselCapacities);
        ...
        var vesselDraughtChart = new dc.BarChart('#draught-chart').width(chartWidth).height(chartHeight).margins({
          ...
          return chartNbBars;
        }).brushOn(false).xAxisLabel("Tirant d'eau (m)").elasticY(true).dimension(vesselDraught).group(vesselDraughts);
        vesselDraughtChart.yAxis().ticks(6);
        vesselDraughtChart.render();
        var vesselTypeX = ndx.dimension(function (d) {
          return d.vesselClass;
        });
        var types = vesselTypeX.group().all().map(function (d) {
          return d.key;
        });
        ...
        var typeColorScale = d3.scaleOrdinal(d3.schemeSet2).domain(types);
        ...
        var vesselTypeChart = new dc.BarChart("#type-chart").x(d3.scaleOrdinal().domain([0, 0])).width(200).height(750).margins({
          ...
          bottom: 180,
          left: 0
        }).dimension(vesselTypeY).group(vesselTypesY, firstType, function (d) {
          return d.value[firstType];
        }).xUnits(function () {
          return 1;
...
//# sourceMappingURL=/onglet2.e703e30f.js.map